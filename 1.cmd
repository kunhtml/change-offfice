...
:: Check if Null service is working
...
:: Log start
echo [%DATE% %TIME%] Starting script execution >> "%temp%\MAS_log.txt"
...
cmd /c "%psc% "$f=[io.file]::ReadAllText('!_batp!') -split ':PStest:\s*';iex ($f[1])"" | find /i "FullLanguage" %nul1% || (
    %eline%
    echo PowerShell is not functioning correctly or is disabled.
    echo Please ensure PowerShell is enabled and not restricted by policy.
    echo:
...
:: Disable QuickEdit and launch from conhost.exe if needed
if %winbuild% GEQ 17763 (set terminal=1) else (set terminal=)
if defined terminal (
    %psc% "$hIcon = [AppDomain]::CurrentDomain.DefineDynamicAssembly(4, 1).DefineDynamicModule(2, $False).DefineType(0); $hIcon.DefinePInvokeMethod('GetConsoleWindow', 'kernel32.dll', 22, 1, [IntPtr], @(), 1, 3).SetImplementationFlags(128); $hIcon.DefinePInvokeMethod('SendMessageW', 'user32.dll', 22, 1, [IntPtr], @([IntPtr], [UInt32], [IntPtr], [IntPtr]), 1, 3).SetImplementationFlags(128); $hWnd = $hIcon.CreateType()::GetConsoleWindow(); echo $($hIcon.CreateType()::SendMessageW($hWnd, 127, 0, 0) -ne [IntPtr]::Zero);" %nul2% | find /i "True" %nul1% && set terminal=
)
...
%launchcmd% "$t=[AppDomain]::CurrentDomain.DefineDynamicAssembly(4, 1).DefineDynamicModule(2, $False).DefineType(0); $t.DefinePInvokeMethod('GetStdHandle', 'kernel32.dll', 22, 1, [IntPtr], @([Int32]), 1, 3).SetImplementationFlags(128); $t.DefinePInvokeMethod('SetConsoleMode', 'kernel32.dll', 22, 1, [Boolean], @([IntPtr], [Int32]), 1, 3).SetImplementationFlags(128); $k=$t.CreateType(); $b=$k::SetConsoleMode($k::GetStdHandle(-10), 0x0080); & cmd.exe '/c' '!_PSarg! -qedit'" && (exit /b) || (set terminal=1)
...
for %%A in (_oArch _updch _lang _clversion _version _oIds _c2rXml _c2rExe _c2rCexe _m) do (
    if not defined %%A (
        %eline%
...
cls
call :oe_chkinternet
if not defined _int goto :oe_goback

cls
if not defined terminal mode 76, 25
if %change%==1 (title Change all editions %masver%) else (title Add edition %masver%)
...
for %%# in (Access Excel Lync OneNote Outlook PowerPoint Project Publisher Visio Word) do (
    if defined suites (
...
:excludelist
set excludelist=
for %%# in (access excel onenote outlook powerpoint project publisher visio word lync onedrive teams) do (
...
if defined projvis (
    for %%# in (ar-sa cs-cz da-dk de-de el-gr en-us es-es fi-fi fr-fr he-il hu-hu it-it ja-jp ko-kr nb-no nl-nl pl-pl pt-br pt-pt ro-ro ru-ru sk-sk sl-si sv-se tr-tr uk-ua zh-cn zh-tw) do (
        if /i "%_lang%"=="%%#" set langmatched=1
    )
    if not defined langmatched (
        %eline%
        echo Language [%_lang%] is not supported for Project/Visio apps.
        echo:
        call :dk_color %Blue% "Install Office in a supported language for Project/Visio from the below URL."
        set fixes=%fixes% %mas%genuine-installation-media
        call :dk_color %_Yellow% "%mas%genuine-installation-media"
        goto :oe_goback
    )
...
set "c2rcommand="%_c2rExe%" platform=%_oArch% culture=%_lang% productstoadd=%targetedition%.16_%_lang%_x-none"
set "c2rcommand=%c2rcommand% cdnbaseurl.16=http://officecdn.microsoft.com/pr/%_updch% baseurl.16=http://officecdn.microsoft.com/pr/%_updch%"
set "c2rcommand=%c2rcommand% version.16=%_version% mediatype.16=CDN sourcetype.16=CDN deliverymechanism=%_updch%"
set "c2rcommand=%c2rcommand% %targetedition%.excludedapps.16=groove%excludelist% flt.useteamsaddon=disabled flt.usebingaddononinstall=disabled flt.usebingaddononupdate=disabled"
if %change%==1 set "c2rcommand=%c2rcommand% productstoremove=AllProducts"

echo: >> "%temp%\MAS_log.txt"
echo [%DATE% %TIME%] Running command: %c2rcommand% >> "%temp%\MAS_log.txt"
echo:
...
if %errorcode% EQU 0 (
    ...
    echo [%DATE% %TIME%] Command executed successfully >> "%temp%\MAS_log.txt"
) else (
    set fixes=%fixes% %mas%troubleshoot
    call :dk_color2 %Blue% "Help - " %_Yellow% " %mas%troubleshoot"
    echo [%DATE% %TIME%] Command failed with error code: %errorcode% >> "%temp%\MAS_log.txt"
)
...
if not defined terminal mode 98, 35
...
cls
if not defined terminal mode 98, 33
...
if exist "%ProgramData%\Microsoft\ClickToRun\ProductReleases\%_actconfig%\x-none.16\MasterDescriptor.x-none.xml" set "_masterxml=%ProgramData%\Microsoft\ClickToRun\ProductReleases\%_actconfig%\x-none.16\MasterDescriptor.x-none.xml"
if exist "%_cfolder%\OfficeClickToRun.exe" set "_c2rExe=%_cfolder%\OfficeClickToRun.exe"
if exist "%_cfolder%\OfficeC2RClient.exe" set "_c2rCexe=%_cfolder%\OfficeC2RClient.exe"
...
if not defined _int (
    %psc% "If([Activator]::CreateInstance([Type]::GetTypeFromCLSID([Guid]'{DCB00C01-570F-4A9B-8D69-199FDBA5723B}')).IsConnectedToInternet){Exit 0}Else{Exit 1}" %nul2%
